...
function getTestDisplayName(test: any, allTests: any[]) {
  const sameNameCount = allTests.filter(
    (t) =>
  ...
  if (sameNameCount <= 1) {
    return String(test?.name || 'Prueba');
  }

  const params = getSortedParameterNamesFromTest(test);

  if (params.length > 0) {
    return params.join(' + ');
  }

  return String(test?.description || test?.name || 'Prueba');
}
                  ...
                  {groupTestIds.map((id) => {
                    ...
                    return (
                      <Badge key={id} variant="outline" className="gap-1">
                        {test ? getTestDisplayName(test, tests) : 'Prueba'}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-semibold text-sm">
                              {getTestDisplayName(test, tests)}
                        </p>

                        {getTestDisplayName(test, tests) !== test.name && (
